...
const CartPosition = connect(null, { removePosition, changePositionCount })(
  (props) => {
    const {
      image,
      id,
    ...
    const handleMinusClick = useCallback(() => {
      if (count.value === 1) {
        removePosition(id);
        console.log("removed: " + cartTitle);
      } else decrementCount();
    }, [incrementCount, count.value, id]);
      ...
      <div className={s._item}>
        ...
        <div className={s._itemInfo}>
          ...
          <div className={s._itemHeader}>
            ...
            <div className={s._itemOption}>{cartItemDescription}</div>
          </div>
          <div className={s._itemFooter}>
            ...
            <div className={s._itemCountWrapper}>
              ...
              >
                <Minus
                  className={cx(s._itemCountIcon, s._itemCountIcon_minus)}
                />
              </button>
              <InputMask
                mask="999"
              ...
              >
                <Plus className={cx(s._itemCountIcon, s._itemCountIcon_plus)} />
              </button>
            </div>
          </div>
        </div>
              ...
              ({ category, positions }, index) => (
                <Fragment key={index}>
                  ...
                  {positions.map((position, index) => (
                    <CartPosition {...position} key={index} />
                  ))}
                </Fragment>
              )
            )}
